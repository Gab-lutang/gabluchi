...
)
{
	...
	public async Task<QuickFixStatus> GetStatusAsync(long appId)
	{
		...
		// Steamless/DRM check - look for packed EXEs
		bool hasBak = new DirectoryInfo(installDir).GetFiles("*.bak").Length > 0;
		if (hasBak)
		{
			// Backup means Steamless was already run — good sign
		}
